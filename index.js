...
// vouches: { [guildId]: { count, channelId } }
const vouchDataRaw = loadJSON(VOUCHES_FILE, {});
const vouchData    = new Map(Object.entries(vouchDataRaw));
...
// Parses duration strings like '1h', '30m', '2d' → milliseconds. Returns null if unparseable.
      ...
      (async () => {
        try {
          ...
          const resultsEmbed = new EmbedBuilder().setColor(0x2ECC71).setTitle(`🎁 ${gw.prize} [RESULTS]`)
            .setDescription(`The winner of this giveaway is tagged above! Congratulations 🎉`)
            .addFields({ name: 'Prize', value: gw.prize }, { name: 'Participants', value: `${participants.length}` })
            .setFooter({ text: `${BOT_NAME} | ${SITE_URL}`, iconURL: client.user.displayAvatarURL() }).setTimestamp();
          await gwCh.send({ content: winner ? `<@${winner}>` : '❌ No participants — no winner.', embeds: [resultsEmbed] });
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          const resultsEmbed = new EmbedBuilder().setColor(0x2ECC71).setTitle(`🎁 ${g.prize} [RESULTS]`)
            .setDescription(`The winner of this giveaway is tagged above! Congratulations 🎉`)
            .addFields({ name: 'Prize', value: g.prize }, { name: 'Participants', value: `${participants.length}` })
            .setFooter({ text: `${BOT_NAME} | ${SITE_URL}`, iconURL: client.user.displayAvatarURL() }).setTimestamp();
          await gwCh.send({ content: winner ? `<@${winner}>` : '❌ No participants — no winner.', embeds: [resultsEmbed] });
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (cmd === 'giveaway') {
        ...
        if (!durMs) return interaction.reply({ content: '❌ Invalid duration. Use formats like `10m`, `1h`, `2d`.', flags: 64 });
        ...
        giveaways.set(msg.id, { prize, channelId: targetCh.id, guildId: interaction.guild.id, endsAt: endsAt.toISOString(), participants: new Set(), ended: false });
        saveGiveaways();
      ...
      if (cmd === 'setupvouch') {
        ...
        const existing = vouchData.get(interaction.guild.id) || { count: 0, channelId: targetCh.id };
        existing.channelId = targetCh.id;
        vouchData.set(interaction.guild.id, existing);
        saveVouches();
        await interaction.reply({ content: `✅ Vouch panel posted in <#${targetCh.id}>`, flags: 64 }); autoDelete(interaction, 5000);
        return;
      }
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId === 'vouch_modal') {
        ...
        const gData = vouchData.get(interaction.guild.id) || { count: 0, channelId: null };
        gData.count += 1;
        vouchData.set(interaction.guild.id, gData);
        saveVouches();

        const vouchCh = gData.channelId
          ? interaction.guild.channels.cache.get(gData.channelId)
          : interaction.channel;
        ...
        if (vouchCh) await vouchCh.send({ embeds: [embed] });
