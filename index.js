...
function canAccessStock(member) {
  ...
  if (member.roles.cache.some(r => r.name === VERIFIED_ROLE)) return true;
  return false;
}
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (cmd === 'gensteam') {
        if (!canAccessStock(interaction.member)) {
          return interaction.reply({ content: `❌ You need the **${VERIFIED_ROLE}** role to generate an account.`, flags: 64 });
        }
        const type = normalizeStockType(interaction.options.getString('type'));
      ...
      if (cmd === 'postgensteam') {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `Click the button below to generate a Steam account.\n\n` +
            `You need the **${VERIFIED_ROLE}** role to use this.\n` +
            `Limit: one account per person every **${STOCK_COOLDOWN_HOURS}h**, per account type.`
          )
          .setFooter({ text: BOT_NAME, iconURL: client.user.displayAvatarURL() });

        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId('gensteam_open').setLabel('Generate Account').setEmoji('🎮').setStyle(ButtonStyle.Primary)
        );
    ...
    if (interaction.isButton()) {
      ...
      // Steam stock generator panel button
