...
// Static useful-links list for /postusefullinks. Edit this array directly in
// the code and redeploy to add/remove/update entries — no bot restart-time
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (cmd === 'postusefullinks') {
        ...
        const description = USEFUL_LINKS.map((l, i) => `**${i + 1}.** [${l.label}](${l.url})`).join('\n');
        ...
        await interaction.reply({ content: `✅ Posted **${USEFUL_LINKS.length}** links in <#${channel.id}>.`, flags: 64 });
        return;
      }
