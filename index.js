...
// Normalizes a channel name for matching: lowercase, strip emoji/symbols/pipes/spaces, keep only letters/numbers/hyphens/underscores.
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (cmd === 'setup-invites') {
        await interaction.deferReply({ ephemeral: true });
        const guild = interaction.guild;
        let invCh = findChannelByName(guild, INVITES_CHANNEL);
    ...
    if (interaction.isButton()) {
      ...
      if (customId === 'get_invite_link') {
        try {
          const invCh = findChannelByName(guild, INVITES_CHANNEL) || guild.channels.cache.find(c => c.type === ChannelType.GuildText);
