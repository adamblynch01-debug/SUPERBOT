...
const USEFUL_LINKS_FILE = path.join(DATA_DIR, 'useful_links.json');
function saveUsefulLinks() { saveJSON(USEFUL_LINKS_FILE, usefulLinks); }
...
let usefulLinks = loadJSON(USEFUL_LINKS_FILE, null);
if (!usefulLinks) {
  usefulLinks = USEFUL_LINKS_SEED;
  saveUsefulLinks();
}
...
function buildUsefulLinksEmbed() {
  const description = usefulLinks.length
    ? usefulLinks.map((l, i) => `**${i + 1}.** [${l.label}](${l.url})`).join('\n')
    : 'No links added yet.';
...
// ─── Redeemable time-limited role keys ─────────────────────────────────────
// keysData is keyed by the key string itself:
//   {
//     roleId, roleName, durationMs, guildId,
//     createdBy, createdAt,
//     status: 'unredeemed' | 'active' | 'expired' | 'revoked',
//     redeemedBy, redeemedAt, expiresAt,
//   }
// Persisted to DATA_DIR so keys, redemptions, and expiry timestamps all
// survive restarts — the sweep below re-derives what to expire from this
// file on every boot, it doesn't rely on an in-memory timer surviving.
const KEYS_FILE = path.join(DATA_DIR, 'keys.json');
function saveKeys() { saveJSON(KEYS_FILE, keysData); }
let keysData = loadJSON(KEYS_FILE, {});

function generateKeyString() {
  const segment = () => crypto.randomBytes(2).toString('hex').toUpperCase();
  let key;
  do { key = `UH-${segment()}-${segment()}-${segment()}`; } while (keysData[key]);
  return key;
}

// Accepts "30m", "12h", "7d", "2w" — returns milliseconds, or null if invalid.
...
async function sweepExpiredKeys() {
  ...
  for (const [key, entry] of Object.entries(keysData)) {
    ...
    try {
      ...
      if (member && role && member.roles.cache.has(role.id)) {
        await member.roles.remove(role).catch(e => console.error(`[keys] failed to remove expired role for key ${key}:`, e));
      }
    } catch (e) {
      console.error(`[keys] sweep error for key ${key}:`, e);
    }
  }
...
async function redeemKey(interaction, rawKeyInput) {
  ...
  entry.expiresAt  = new Date(now + entry.durationMs).toISOString();
  saveKeys();

  const expiresUnix = Math.floor((now + entry.durationMs) / 1000);
  return interaction.reply({
    content: `🎉 Key redeemed! You've been given **${role.name}** until <t:${expiresUnix}:F> (<t:${expiresUnix}:R>).`,
    flags: 64,
  });
}
...
const ownCommands = [
  ...
  new SlashCommandBuilder().setName('genkey').setDescription('Staff: Generate redeemable key(s) that grant a role for a set duration')
    ...
    .addStringOption(o => o.setName('duration').setDescription('How long the role lasts, e.g. 30m, 12h, 7d, 2w').setRequired(true))
...
client.once('ready', async () => {
  ...
  startAuthServer(client, { issueKey: issueKeyAndNotify });
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (cmd === 'postusefullinks') {
        ...
        const channel = interaction.options.getChannel('channel') || interaction.channel;
        await channel.send({ embeds: [buildUsefulLinksEmbed()] });
        await interaction.reply({ content: `✅ Posted **${usefulLinks.length}** links in <#${channel.id}>.`, flags: 64 });
        return;
      }
      ...
      if (cmd === 'addusefullink') {
        ...
        usefulLinks.push({ label: title, url });
        saveUsefulLinks();

        return interaction.reply({
          content: `✅ Added **${title}** as link **#${usefulLinks.length}**. Run \`/postusefullinks\` to repost the updated list.`,
          flags: 64,
        });
      }
      ...
      if (cmd === 'listusefullinks') {
        if (!hasAccess(interaction)) return interaction.reply({ content: '❌ No permission.', flags: 64 });
        return interaction.reply({ embeds: [buildUsefulLinksEmbed()], flags: 64 });
      }
      ...
      if (cmd === 'clearusefullinks') {
        ...
        const removed = usefulLinks.length;
        usefulLinks = [];
        saveUsefulLinks();
      ...
      if (cmd === 'importusefullinks') {
        ...
        if (mode === 'replace') {
          ...
          usefulLinks.push(...parsed);
        }
        saveUsefulLinks();

        let msg = `✅ ${mode === 'replace' ? 'Replaced the list with' : 'Added'} **${parsed.length}** link${parsed.length === 1 ? '' : 's'}. Total now: **${usefulLinks.length}**.`;
        if (skipped.length) {
      ...
      if (cmd === 'genkey') {
        ...
        if (!durationMs) {
          return interaction.reply({ content: '❌ Invalid duration. Use formats like `30m`, `12h`, `7d`, `2w`.', flags: 64 });
        }
        ...
        return interaction.reply({
          content: `✅ Generated **${amount}** key${amount === 1 ? '' : 's'} for **${role.name}** (${durationStr}):\n` +
      ...
      if (cmd === 'listkeys') {
        ...
        const entries     = Object.entries(keysData).filter(([, e]) => e.guildId === interaction.guild.id);
        const unredeemed  = entries.filter(([, e]) => e.status === 'unredeemed');
        const active      = entries.filter(([, e]) => e.status === 'active');

        const fmtUnredeemed = unredeemed.length
          ? unredeemed.slice(0, 15).map(([k, e]) => `\`${k}\` — ${e.roleName}`).join('\n') +
            (unredeemed.length > 15 ? `\n...and ${unredeemed.length - 15} more` : '')
          : 'None';

        const fmtActive = active.length
          ? active.slice(0, 15).map(([, e]) =>
              `<@${e.redeemedBy}> — ${e.roleName} — expires <t:${Math.floor(new Date(e.expiresAt).getTime() / 1000)}:R>`
