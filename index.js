  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (cmd === 'setupvouch') {
        ...
        const existing = vouchData.get(interaction.guild.id) || { count: 0, channelId: resultsCh.id };
    ...
    if (interaction.isButton()) {
      ...
      if (customId === 'leave_vouch') {
        ...
        modal.addComponents(
          ...
          new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('vouch_image').setLabel('IMAGE URL (optional)').setStyle(TextInputStyle.Short).setRequired(false).setMaxLength(500).setPlaceholder('https://...')),
        );
        return interaction.showModal(modal);
      }
