...
client.on('guildMemberAdd', async member => {
  ...
  // Welcome card
  try {
    const welcomeCh = member.guild.channels.cache.find(c => c.name === WELCOME_CHANNEL && c.type === ChannelType.GuildText);
    if (!welcomeCh) return;
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (cmd === 'setup-verify') {
        ...
        // Channels Verified role is allowed to see (besides get-verify)
        const VERIFIED_ALLOWED = [
          process.env.SUPPORT_CHANNEL_NAME || '🚨-support',
          'support 1',
        ];
        for (const [, ch] of guild.channels.cache) {
          ...
          try {
            ...
            // Only grant Verified view access to the allowed channels
            const chNameLower = ch.name.toLowerCase();
            const isAllowed = VERIFIED_ALLOWED.some(n => chNameLower.includes(n.toLowerCase()));
            if (isAllowed) {
        ...
        await verifyCh.permissionOverwrites.edit(verifiedRole, { ViewChannel: false });
