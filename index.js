  ...
  try {
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId === 'postlink_modal' || interaction.customId.startsWith('postlink_modal::')) {
        ...
        if (stashed) {
          const found = interaction.guild.channels.cache.find(c => c.name === stashed || c.id === stashed);
          if (found) targetCh = found;
        }
        ...
        const body =
          `> **${title}**\n` +
          `> ${url}\n` +
          `> ${BOT_NAME}${SITE_URL ? ` | ${SITE_URL}` : ''} | ${timestamp}`;

        try {
          // Use a webhook so the message shows the bot's name + avatar as the author,
          // matching the "Esp Gang" style posts shown in the reference screenshots.
          if (!targetCh.isTextBased() || !('fetchWebhooks' in targetCh)) {
            throw new Error('That channel does not support webhooks (try a regular text channel).');
          }
          const webhooks = await targetCh.fetchWebhooks();
          let hook = webhooks.find(w => w.owner?.id === client.user.id);
          if (!hook) {
            hook = await targetCh.createWebhook({
              name: BOT_NAME,
              avatar: client.user.displayAvatarURL(),
              reason: 'Used for /postlink posts',
            });
          }
          await hook.send({ content: body, username: BOT_NAME, avatarURL: client.user.displayAvatarURL() });
