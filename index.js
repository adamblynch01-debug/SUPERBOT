...
async function getGuildSettings(guildId) {
  ...
  const data = {
    verifiedRole:      row?.verified_role_name   || (isOriginal ? VERIFIED_ROLE_ENV      : 'Verified'),
    verifyChannel:      row?.verify_channel_name  || (isOriginal ? VERIFY_CHANNEL_ENV     : 'get-verify'),
    welcomeChannel:     row?.welcome_channel_name || (isOriginal ? WELCOME_CHANNEL_ENV    : 'welcome'),
    invitesChannel:     row?.invites_channel_name || (isOriginal ? INVITES_CHANNEL_ENV    : 'invites'),
...
client.on('guildMemberAdd', async member => {
  ...
  try {
    await member.send(
      `👋 Welcome to **${member.guild.name}**!\n\nPlease head to **#${settings.verifyChannel}** and click **Verify Me** to access the server.`
    );
  } catch (_) {}

  // Welcome card
  try {
    const welcomeCh = member.guild.channels.cache.get(WELCOME_CHANNEL_ID) || findChannelByName(member.guild, settings.welcomeChannel);
    if (!welcomeCh) return;
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (cmd === 'setup-verify') {
        ...
        let verifiedRole = guild.roles.cache.find(r => r.name === settings.verifiedRole);
        if (!verifiedRole) verifiedRole = await guild.roles.create({ name: settings.verifiedRole, color: 0x5865f2 });
        ...
        for (const [, ch] of guild.channels.cache) {
          if (ch.name === settings.verifyChannel) continue;
          try {
        ...
        let verifyCh = findChannelByName(guild, settings.verifyChannel);
        if (!verifyCh) verifyCh = await guild.channels.create({ name: settings.verifyChannel, type: ChannelType.GuildText });
      ...
      if (cmd === 'setup-invites') {
        ...
        const guild = interaction.guild;
        const settings = await getGuildSettings(guild.id);
        let invCh = guild.channels.cache.get(INVITES_CHANNEL_ID) || findChannelByName(guild, settings.invitesChannel);
        if (!invCh) invCh = await guild.channels.create({ name: settings.invitesChannel, type: ChannelType.GuildText });
      ...
      if (cmd === 'importvouches') {
        ...
        const vouchCh = gData.channelId
          ? (interaction.guild.channels.cache.get(gData.channelId) || (ivSettings.vouchesChannelId && interaction.guild.channels.cache.get(ivSettings.vouchesChannelId)))
      ...
      if (cmd === 'set-tos' || cmd === 'set-rules' || cmd === 'set-guide') {
        if (!hasAccess(interaction)) return interaction.reply({ content: '❌ No permission.', flags: 64 });
      ...
      if (cmd === 'post-tos' || cmd === 'post-rules' || cmd === 'post-guide') {
        if (!hasAccess(interaction)) return interaction.reply({ content: '❌ No permission.', flags: 64 });
    ...
    if (interaction.isButton()) {
      ...
      if (customId === 'verify_button') {
        const verifiedRole = guild.roles.cache.find(r => r.name === btnSettings.verifiedRole);
      ...
      if (customId === 'get_invite_link') {
        try {
          const invCh = guild.channels.cache.get(INVITES_CHANNEL_ID) || findChannelByName(guild, btnSettings.invitesChannel) || guild.channels.cache.find(c => c.type === ChannelType.GuildText);
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId === 'vouch_modal') {
        ...
        const vouchCh = gData.channelId
          ? (interaction.guild.channels.cache.get(gData.channelId) || (vouchSettings.vouchesChannelId && interaction.guild.channels.cache.get(vouchSettings.vouchesChannelId)))
