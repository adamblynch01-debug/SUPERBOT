...
function registerPanelRoutes(app, discordClient, { invalidateGuildSettings } = {}) {
  ...
  // ── API: TOS / Rules / Guide content ────────────────────────────────────
  app.get('/panel/api/guilds/:guildId/content/:key', requireSession, async (req, res) => {
    const guildId = assertGuildAccess(req, res);
    if (!guildId) return;
    if (!['tos', 'rules', 'guide', 'payment-method'].includes(req.params.key)) return res.status(400).json({ error: 'Invalid content key.' });
    const { rows } = await db.query(
      'SELECT title, body, updated_at FROM guild_content WHERE guild_id = $1 AND content_key = $2',
      [guildId, req.params.key]
    );
    res.json({ content: rows[0] || null });
  });

  app.post('/panel/api/guilds/:guildId/content/:key', requireSession, async (req, res) => {
    const guildId = assertGuildAccess(req, res);
    if (!guildId) return;
    if (!['tos', 'rules', 'guide', 'payment-method'].includes(req.params.key)) return res.status(400).json({ error: 'Invalid content key.' });
    ...
    await db.ensureGuild(guildId);
    await db.query(
      `INSERT INTO guild_content (guild_id, content_key, title, body, updated_at)
       VALUES ($1,$2,$3,$4, now())
       ON CONFLICT (guild_id, content_key) DO UPDATE SET title = $3, body = $4, updated_at = now()`,
      [guildId, req.params.key, title, body]
    );
    res.json({ success: true });
  });
...
function renderPanelPage(session, selectedGuildId) {
  const CONTENT_LABELS = { tos: 'Terms of Service', rules: 'Rules', guide: 'Guide', 'payment-method': 'Payment Methods' };
  const SHARED_HEAD = `
      ...
      <div class="dash">
        ...
        <div class="content">
          ...
          <div class="section" id="section-content">
            ${['tos', 'rules', 'guide', 'payment-method'].map(key => `
            <div class="win">
              <div class="win-head"><span class="prompt">$</span><span class="cmd">vim ${key}.md</span></div>
              <div class="win-body">
                <div class="field"><label>Title</label><input id="content_${key}_title" placeholder="Loading..."></div>
                <div class="field"><label>Body</label><textarea id="content_${key}_body" rows="6" style="width:100%;background:var(--panel);border:1px solid var(--border);color:var(--text);padding:10px 12px;border-radius:6px;font-family:inherit;font-size:13px;resize:vertical"></textarea></div>
                <div style="margin:10px 2px"><button onclick="saveContent('${key}')">&gt; Save ${CONTENT_LABELS[key]}</button></div>
              </div>
            </div>`).join('')}
          </div>
  ...
  <script>
    ...
    async function loadContent(key) {
      const res = await fetch('/panel/api/guilds/' + GUILD_ID + '/content/' + key);
      const data = await res.json();
      const c = data.content;
      document.getElementById('content_' + key + '_title').value = c ? c.title : '';
      document.getElementById('content_' + key + '_title').placeholder = c ? '' : 'e.g. 📜 Terms of Service';
      document.getElementById('content_' + key + '_body').value = c ? c.body : '';
    }

    async function saveContent(key) {
      const title = document.getElementById('content_' + key + '_title').value.trim();
      const body = document.getElementById('content_' + key + '_body').value.trim();
      if (!title || !body) return alert('Both title and body are required.');
      await fetch('/panel/api/guilds/' + GUILD_ID + '/content/' + key, {
        method: 'POST', headers: {'Content-Type':'application/json'},
        body: JSON.stringify({ title, body })
      });
      alert('Saved. Run /post-' + key + ' in Discord to post it.');
    }

    loadLinks(); loadStock(); loadKeys(); loadTickets(); loadSettings();
    loadContent('tos'); loadContent('rules'); loadContent('guide'); loadContent('payment-method');
  </script>
  </body></html>`;
}
