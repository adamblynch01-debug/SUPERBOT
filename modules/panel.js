...
function registerPanelRoutes(app, discordClient, { invalidateGuildSettings } = {}) {
  ...
  app.get('/panel/api/guilds/:guildId/content/:key', requireSession, async (req, res) => {
    const guildId = assertGuildAccess(req, res);
    if (!guildId) return;
    if (!['tos', 'rules', 'guide'].includes(req.params.key)) return res.status(400).json({ error: 'Invalid content key.' });
    const { rows } = await db.query(
...
function renderPanelPage(session, selectedGuildId) {
  const CONTENT_LABELS = { tos: 'Terms of Service', rules: 'Rules', guide: 'Guide' };
  const SHARED_HEAD = `
  ...
  <script>
    ...
    loadContent('tos'); loadContent('rules'); loadContent('guide');
  </script>
  </body></html>`;
}
