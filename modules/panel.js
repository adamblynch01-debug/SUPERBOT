...
function renderPanelPage(session) {
  ...
  const guildOptions = session.manageableGuilds.map(g => `<option value="${g.id}">${g.name}</option>`).join('');
  const noGuilds = session.manageableGuilds.length === 0
    ? `<div class="notice">No manageable servers found — you need <b>Manage Server</b> permission and the bot must already be installed there. <a href="/panel/install">Add the bot to a server</a>.</div>`
    : '';
  ...
  <body>
    <div class="wrap">
      <div class="chrome"><span class="dot r"></span><span class="dot y"></span><span class="dot g"></span>
        <span class="chrome-path"><b>${session.username}</b>@superbot:~/panel$</span></div>

      <div class="topbar">
        <span class="who">logged in as <b>${session.username}</b></span>
      ...
      <div class="win">
        ...
        <div class="win-body">
          <div class="warn">✅ These now apply live — changes take effect within ~30s (or instantly for most edits). Verify role/channel, welcome channel, invites, counting channel, and Gen Member/OVERSEER roles are all wired up. Anti-scam thresholds and log/staff/ticket channel IDs save here too but are not yet read dynamically by every module — double check those still behave as expected.</div>
  ...
  <script>
    ...
    async function saveSettings() {
      ...
      alert('Saved — most settings apply within ~30 seconds. See the note above for which fields are fully wired up yet.');
    }
