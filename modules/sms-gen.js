...
async function smspoolGetServices(apiKey) {
  const d = await safeFetch(`${SMSPOOL_BASE}/service/retrieve_all`, { method: 'POST' });
...
async function smspoolGetCountries(apiKey, serviceId) {
  const d = await safeFetch(`${SMSPOOL_BASE}/country/retrieve_all`, { method: 'POST' });
  if (!Array.isArray(d)) throw new Error(d.message || 'Failed to fetch countries');
  return d.map(c => ({ label: c.name.slice(0, 100), value: String(c.ID) }));
}

async function smspoolBuyNumber(apiKey, serviceId, countryId) {
  const purchaseParams = new URLSearchParams({ key: apiKey, service: serviceId, country: countryId });
  const d = await safeFetch(`${SMSPOOL_BASE}/sms/purchase`, { method: 'POST', body: purchaseParams });
  if (d.success !== 1) throw new Error(d.message || 'Purchase failed — no numbers available for this country/service combo');
  return { orderId: String(d.order_id), number: d.phonenumber || d.number };
}

async function smspoolCheckSMS(apiKey, orderId) {
  const d = await safeFetch(`${SMSPOOL_BASE}/request/active?key=${apiKey}&orderid=${orderId}`);
  // /request/active: status = "pending" | "completed" | "expired" | "refunded"
  // sms field contains the code when completed
  const code = d.sms && d.sms !== '0' ? String(d.sms) : null;
...
async function handleSMSInteraction(interaction, client) {
  ...
  // ── Step 2: Provider chosen → fetch ALL services, show page 0 ──────────────
  if (interaction.isStringSelectMenu() && interaction.customId === 'sms_pick_provider') {
    ...
    const row   = buildPagedMenu(`sms_pick_service__${provider}__0`, services, 0, `2️⃣  Choose a service... (${total} total)`);
