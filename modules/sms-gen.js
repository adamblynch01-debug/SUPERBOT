...
async function safeFetch(url, opts = {}) {
  ...
  try {
    ...
    // Provider returned plain text (e.g. "no free phones") — wrap it
...
async function smspoolGetServices(apiKey) {
  const d = await safeFetch(`${SMSPOOL_BASE}/service/retrieve_all_services?key=${apiKey}`);
...
async function smspoolGetCountries(apiKey, serviceId) {
  const d = await safeFetch(`${SMSPOOL_BASE}/country/retrieve_all_countries?key=${apiKey}&service=${serviceId}`);
...
async function smspoolBuyNumber(apiKey, serviceId, countryId) {
  const d = await safeFetch(`${SMSPOOL_BASE}/sms/purchase?key=${apiKey}&service=${serviceId}&country=${countryId}`);
...
async function smspoolCancel(apiKey, orderId) {
  await safeFetch(`${SMSPOOL_BASE}/sms/cancel?key=${apiKey}&orderid=${orderId}`).catch(() => {});
}

async function smspoolResend(apiKey, orderId) {
  const d = await safeFetch(`${SMSPOOL_BASE}/sms/resend?key=${apiKey}&orderid=${orderId}`).catch(() => ({}));
  return d.success === 1;
}
