...
// Mirrors index.js's parseKeyDuration — kept in sync manually since this
// module doesn't import index.js's internals. "lifetime" returns the string
// sentinel 'lifetime' (never expires); invalid input returns null.
...
function startAuthServer(discordClient, { issueKey } = {}) {
  registerPanelRoutes(app, discordClient);
  ...
  app.post('/api/keys/issue', async (req, res) => {
    ...
    if (durationMs === null) {
      return res.status(400).json({ error: 'Invalid duration format. Use 30m, 12h, 7d, 2w, or "lifetime".' });
    }
